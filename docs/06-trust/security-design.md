...
| 단계 | 처리 주체 | 허용하는 것 |
|---|---|---|
| 카카오싱크 | Kakao + 서비스 약관 | 로그인, 계정 식별과 등록된 제공 정보 |
| 서비스 위치 선택 동의 | 빵찾깅 | 위치 이용 목적·Kakao 전송·비저장 안내 확인 |
| 브라우저 위치 권한 | 브라우저·OS | 실제 GPS 센서 접근 |

카카오싱크 체크박스가 GPS 접근을 허용한다고 설명하지 않는다. 위치는 선택이며 거부해도 역·동·구 입력으로 추천을 완료할 수 있다.
...
리뷰 암호문은 AES-256-GCM, 중복 키는 별도 HMAC-SHA-256을 사용한다. encryption key와 dedupe key를 분리하고 OS 비밀 저장소에서 버전 관리한다. nonce 재사용 0건과 tag 검증 실패 즉시 중단을 검사한다.
