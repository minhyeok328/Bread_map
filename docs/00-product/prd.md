...
| ID | 요구사항 | P0 수용 기준 |
|---|---|---|
| `FR-AUTH-01` | 사용자는 카카오싱크로 로그인해야 서비스를 이용할 수 있다. | 비로그인 추천·대화 API는 인증 오류를 반환한다. |
...
| `FR-DATA-03` | 리뷰 수집은 관리자 로컬 실험으로만 실행한다. | 일반 사용자 경로에 없고 접근 거부 시 즉시 중단한다. |
...
2. **비공개 P0:** 카카오싱크, 멀티턴 추천, 지도·경로, 대화 기록과 실패 대체를 5명에게 제공한다.
