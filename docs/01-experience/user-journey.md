...
- 카카오싱크 약관 동의와 브라우저 GPS 권한은 별개 단계다.
...
2. `카카오로 시작하기`를 선택한다.
3. 카카오싱크 간편가입 화면에서 서비스 약관과 필요한 카카오 제공 정보를 확인한다.
...
카카오싱크 체크박스는 브라우저 GPS 권한을 허용하지 않는다. 위치 사용을 서비스 가입 필수 동의로 표현하지 않는다.
